...
int Snake::col() const
{
    return m_col;  // Snake's column location
}
...
{
    return m_row;  // Player's row location
}

int Player::col() const
{
    return m_col;  //Player's col location
}

int Player::age() const
{
    return m_age;  //Player's age
}
...
void Player::move(int dir)
{
    ...
    int rChange = 0;
    int cChange = 0;
    directionToDeltas(dir, rChange, cChange);
    int newRow=m_row+rChange;
    int newCol=m_col+cChange;
    
    if(newRow>=1 && newRow<=m_pit->rows() && newCol>=1 && newCol<=m_pit->cols())
    {
        if(m_pit->numberOfSnakesAt(newRow, newCol)==0)
        {
            m_row=newRow;
            m_col=newCol;
        }
        else if(m_pit->numberOfSnakesAt(newRow, newCol)>0 && (newRow+rChange)>=1 && (newRow+rChange)<=m_pit->rows() && (newCol+cChange)>=1 && (newCol+cChange)<=m_pit->cols())
        {
            if(m_pit->numberOfSnakesAt(newRow+rChange, newCol+cChange)!=0)
            {
                m_row=newRow+rChange;
                m_col=newCol+cChange;
                setDead();
            }
            else
            {
                m_row=newRow+rChange;
                m_col=newCol+cChange;
                m_pit->destroyOneSnake(newRow, newCol);
            }
        }
    }
}
...
{
    return m_dead;  // Player's dead or alive status
}
...
int Pit::rows() const
{
    return m_rows;  //Number of rows in Pit
}

int Pit::cols() const
{
    return m_cols;  //Number of cols in Pit
}

Player* Pit::player() const
{
    return m_player;  //Player in Pit
}
...
void Pit::display(string msg) const
{
    ...
    for(int x=0; x<rows(); x++)
    {
        ...
        {
            if(numberOfSnakesAt(x+1, y+1)==1)
            {
                grid[x][y]='S';
            }
            else if(numberOfSnakesAt(x+1, y+1)>=2 && numberOfSnakesAt(x+1, y+1)<=8)
            {
                grid[x][y]=numberOfSnakesAt(x+1,y+1) + '0';
            }
            else if(numberOfSnakesAt(x+1, y+1)>=9)
            {
                grid[x][y]='9';
            }
        }
...
bool Pit::destroyOneSnake(int r, int c)
{
    for(int k=0; k<m_nSnakes; k++)
    {
        ...
        {
            delete m_snakes[k];
            for(int y=k; y<m_nSnakes-1; y++)
            {
                m_snakes[y]=m_snakes[y+1];
            }
            m_nSnakes--;
            return true;
        }
